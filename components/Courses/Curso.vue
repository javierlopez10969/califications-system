                ...
                <v-dialog v-model="dialog" width="500">
                  <v-card>
                    ...
                    <v-list-item v-if="!existe_apelacion">
                        <v-list-item-content>
                        <v-list-item-title>
                            Apelar evaluacion <v-icon @click="mostrarAp=!mostrarAp">mdi-information</v-icon>
                        </v-list-item-title>
                        <div v-if="mostrarAp">
                            <v-list-item-subtitle class="mb-2" >
                                Motivo
                            </v-list-item-subtitle>
                            <v-textarea outlined v-model="descripcionAp" class="mb-0"></v-textarea>
                            <v-btn 
                            class="mt-0" outlined x-small color="primary"
                            @click="confirmar = true">Enviar</v-btn>
                        </div>
                        </v-list-item-content>
                    </v-list-item>
                    <v-list-item v-else>
                        ...
                        <v-list-item-subtitle class="mb-1">Motivo: {{apelacion_actual.description}}</v-list-item-subtitle>
                        <v-list-item-subtitle class="mb-1">Fecha: {{apelacion_actual.issue_date}}</v-list-item-subtitle>
                        <div  v-if="apelacion_actual.answer === ''">
                            <v-list-item-subtitle class="mb-1">Respuesta: Pendiente</v-list-item-subtitle>    
                        </div>
                        <dir v-else>
                            <v-list-item-subtitle class="mb-1">Respuesta: </v-list-item-subtitle>
                            <v-textarea
                                filled readonly outlined
                    ...
                    <v-card-actions>
                      <v-spacer></v-spacer>
                      <v-btn color="primary" text @click="cerrarAp">
                        Cerrar
                      </v-btn>
                    </v-card-actions>
                  </v-card>
...
export default {
  ...
  methods: {
    ...
    obtenerApelacion(id_calificacion, nombre_evaluacion, fecha_de_entrega){
        ...
        this.dialog = true;
        this.$axios.get(
            process.env.baseUrl + "appeals/detail/" +
            id_calificacion + "/"
        ).then((res) => {
